...
public struct FHIRDate: DateAndTime {
    ...
    public init?(string: String) {
        let dt = DateAndTimeParser.shared.parse(string: string)
...
public struct FHIRTime: DateAndTime {
     ...
     */
    public init?(string: String) {
        let dt = DateAndTimeParser.shared.parse(string: string, isTimeOnly: true)
        guard let time = dt.time else {
            return nil
        }

        self.init(hour: time.hour, minute: time.minute, second: time.second, secondsFromString: time.tookSecondsFromString)
    }
...
public struct DateTime: DateAndTime {
    ...
    public init?(string: String) {
        ...
        self.init(date: date, time: dt.time, timeZone: dt.tz)
...
public struct Instant: DateAndTime {
    ...
    public init?(string: String) {
        let dt = DateAndTimeParser.shared.parse(string: string)
        guard let date = dt.date, let time = dt.time, let tz = dt.tz else {
            return nil
        }
        self.init(date: date, time: time, timeZone: tz)
        self.timeZoneString = dt.tzString
    }
                    ...
                    if nil != scanner.fhir_scanString("Z") {
                        ...
                        if let hourStr = scanner.fhir_scanCharacters(from: digitSet) {
                            ...
                            if 2 == hourStr.count {
                                ...
                                let index = hourStr.index(hourStr.startIndex, offsetBy: 2)
                                tzhour = Int(String(hourStr[..<index]))!
                                tzmin = Int(String(hourStr[index...]))!
                            }
